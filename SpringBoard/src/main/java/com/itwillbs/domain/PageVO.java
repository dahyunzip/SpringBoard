...
// 페이징 처리를 위해서 생성한 객체
// 페이징 처리는 우리가 별도의 조작을 해야하므로, lombok @Data 설정 X

public class PageVO {
	
	private int totalCount; 	// 총 글의 개수
	private int startPage;		// (페이징블럭) 시작페이지 번호
	private int endPage;		// (페이징블럭) 끝 페이지 번호
	
	private boolean prev;		// 이전페이지 사용여부
	private boolean next;		// 다음페이지 사용여부

	private int pageBlock = 10;	// 페이징블럭의 크기 1...10 11...20
	
	//private int page;			// 페이지 번호
	//private int pageSize;		// 페이지 출력 개수
	...
	private void calcPageData() {
		...
		// endPage 계산
		// (int)(Math.ceil(page/(double)pageBlock) * pageBlock)
		...
		// prev 계산 (시작페이지를 1인지 아닌지 체크. 시작페이지가 1이면 이전버튼X)
		prev = startPage != 1;
